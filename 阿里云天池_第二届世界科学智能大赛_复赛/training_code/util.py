...
def api_retry(MODEL_NAME, data, temperature, local: bool, use_check=False):
    max_retries = 5
    ...
    while attempts < max_retries:
        try:
            if use_check:
                if local:
                    return call_api_local_check(MODEL_NAME, data, temperature)
                else:
                    return call_api_remote_check(MODEL_NAME, data, temperature)
            else:
                if local:
                    return call_api_local(MODEL_NAME, data["prompt"], temperature)
                else:
                    return call_api_remote(MODEL_NAME, data["prompt"], temperature)
...
def inference_detail(datas, MODEL_NAME, local, temperature, vote_num=1, use_check=False, ):
    inference_res = {}
        ...
        for data in tqdm(datas, desc="Submitting tasks", total=len(datas)):
            for i in range(0, vote_num):
                if isinstance(MODEL_NAME, str):
                    future1 = executor.submit(api_retry, MODEL_NAME, data, local, use_check, temperature)
                    future_data[future1] = (data, data['id'])
                    time.sleep(0.5)
                elif isinstance(MODEL_NAME, list):
                    for MODEL_NAME_sub in MODEL_NAME:
                        future1 = executor.submit(api_retry, MODEL_NAME_sub, data, local, use_check, temperature)
                        future_data[future1] = (data, data['id'])
                        time.sleep(0.5)
